...
Subcommands:
  build           Build VMs and disk images. Run '$SCRIPT_NAME build --help' for more info.
  run <vm-name>   Start VM by its unique name (including the index). Make sure to run build before this.
  runall          Start all VMs built in the last successful build.
  list            List all running QEMU virtual machines (started from this project), one per line.
  stop <vm-name>  Stop the running virtual machine using its unique name.
  ssh <vm-name>   SSH into a VM with provided unique name.
  path            Print nix store path where the current devshell files are.
...
Options:
  -r | --result <result-dir-path>    Relative path to the generated result dir symlink to use, including the result link name (not just the parent directory).
                                     ...
                                     Default: 'result' in this project's root directory (it's already gitignored there).
EOF
)

RESULT_DIR_PATH="$NO_HEADACHE_PROJECT_ROOT_DIR/result"
...
case "$1" in
  ...
  ssh)
    ...
    "$RESULT_DIR_PATH/bin/ssh-into-$2"
    exit 0
  ;;
